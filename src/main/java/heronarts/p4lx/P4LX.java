...
/**
 * Harness to run LX inside a Processing 4 sketch
 */
public class P4LX extends LX {
  ...
  public P4LX(PApplet applet) {
    this(applet, new LXModel());
  }

  public P4LX(PApplet applet, LXModel model) {
    this(applet, new Flags(applet), model);
  }

  public P4LX(PApplet applet, Flags flags) {
    this(applet, flags, null);
  }

  public P4LX(PApplet applet, Flags flags, LXModel model) {
    super(flags, model);
    this.flags = flags;
    this.applet = applet;
    ...
    LX.initProfiler.log("P4LX: registerMethod");

    if (flags.headless) {
      this.ui = null;
    } else {
      this.ui = buildUI();
      LX.initProfiler.log("P4LX: UI");
    }

    applet.colorMode(PConstants.HSB, 360, 100, 100, 100);
    LX.initProfiler.log("P4LX: colorMode");
  }
  ...
  private static final String P4LX_PREFIX = "P4LX";

  public static void log(String message) {
    LX._log(P4LX_PREFIX, message);
  }

  public static void error(Exception x, String message) {
    LX._error(P4LX_PREFIX, x, message);
  }

  public static void error(String message) {
    LX._error(P4LX_PREFIX, message);
  }
}